...
func challengesForTwoWeek(number: Int) -> [Challenge] {
    let randomIndices = generateUniqueRandomNumbers(count: 9, range: (0+number)...(8+number))

    return randomIndices.enumerated().map { (index, randomIndex) -> Challenge in
        let challengeInfo = challenges[randomIndex]
        return Challenge(
            challengeIndex: index,
            challengeTitle: challengeInfo.0,
            challengeDescription: challengeInfo.1,
            progress: 0,
            completed: false,
            buttonClicked: false
        )
...
let randomChallenges = challengesForTwoWeek(number: 0)
